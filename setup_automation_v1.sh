...
echo "⚙️ Setter opp VikingBoard automasjon..."

PROJECT_ROOT="$(pwd)"
echo "Prosjektmappe: $PROJECT_ROOT"

# 1) Init git hvis ikke finnes
if [ ! -d ".git" ]; then
  echo "📦 Initialiserer git-repo..."
  git init
  git branch -M main || true
else
  echo "✅ Git-repo finnes allerede."
fi

# 2) Legg til remote hvis ikke satt
if ! git remote get-url origin >/dev/null 2>&1; then
  echo "🔗 Setter remote til GitHub..."
  git remote add origin https://github.com/tombomann/vikingboard.git
else
  echo "✅ Remote 'origin' er allerede satt."
fi

# 3) .gitignore for KiCad + macOS + Python
...
# 5) Lag dokumentasjons-script (ren Python, ingen pcbnew)
cat <<'PYEOF' > tools/vikingboard_docs.py
#!/usr/bin/env python3
...
chmod +x tools/vikingboard_docs.py

echo "✅ Ferdig med lokal automasjons-setup."
echo "Neste steg blir å sette opp GitHub Actions-workflow."
